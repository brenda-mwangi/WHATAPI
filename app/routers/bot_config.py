...
@router.get("/create-bot/{user_id}", status_code=status.HTTP_200_OK, response_model=schema.UpdateUser, summary="User's Home")
def create_bot(user_update: schema.UpdateUser, user_id: int, db: Session = Depends(get_db), logged_in_user: int = Depends(oath2.get_current_user)):
    """
